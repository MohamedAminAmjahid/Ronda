import { fetchWeeklyLeaderboard, type WeeklyEntry, type GeoFilter } from './client'
...
/** Clé de cache composite ligue + filtre géo (onglets Global/Maroc/France/Ma
 * ville) — une ligue seule ne suffit plus à identifier un jeu de données. */
function cacheKey(league: string, geo?: GeoFilter): string {
  if (geo?.country) return `${league}|country:${geo.country}`
  if (geo?.city) return `${league}|city:${geo.city}`
...
export function getCachedLeaderboard(league: string, geo?: GeoFilter): WeeklyEntry[] | null {
  return cache.get(cacheKey(league, geo))?.data ?? null
}

export function isStale(league: string, geo?: GeoFilter): boolean {
  const entry = cache.get(cacheKey(league, geo))
...
export async function refreshLeaderboard(league: string, geo?: GeoFilter): Promise<void> {
  try {
    const data = await fetchWeeklyLeaderboard(league, geo)
    cache.set(cacheKey(league, geo), { data, fetchedAt: Date.now() })
    notify()
  } catch {
 ...
 * Toujours sans filtre géo (onglet Global, celui affiché par défaut). */
 ...
 * le prochain affichage refetch simplement les ligues/filtres consultés.
 */
export function invalidateLeaderboard(league?: string): void {
  if (league) {
    for (const k of cache.keys()) {
      if (k.startsWith(`${league}|`)) cache.delete(k)
    }
  } else {
    cache.clear()
  }
